	...
	run: async (client, message, args) => {
		try {
			...
			if (channel.userLimit != 0 && channel.full)
				return message.reply({
					embeds: [new MessageEmbed()
						...
						.setTitle(`<:declined:780403017160982538> El canal esta lleno, no puedo entrar`)
					],
				});
			if (channel.guild.me.voice.channel && channel.guild.me.voice.channel.id != channel.id) {
				return message.reply({
					embeds: [new MessageEmbed()
						.setColor(ee.wrongcolor)
						.setFooter(ee.footertext, ee.footericon)
						.setTitle(`<:declined:780403017160982538> Estoy en otro canal con la gente chevere sorry`)
					],
				});
			}
			if (!args[0]) {
				return message.reply({
					embeds: [new MessageEmbed()
						.setColor(ee.wrongcolor)
						.setFooter(ee.footertext, ee.footericon)
						.setTitle(`${client.allEmojis.x} **Porfavor Busca Algo**`)
			...
			try {
				...
				//Edit the reply
				newmsg.edit({
					content: `${queue?.songs?.length > 0 ? "👍 Añadido" : "🎶 Reproduciendo:"}: \`\`\`css\n${Text}\n\`\`\``,
				}).catch(e => {
					console.log(e)
				})
			} catch (e) {
